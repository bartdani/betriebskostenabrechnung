...
def db_with_contract(test_db):
    ...
    a = Apartment(number='DOC_TEST_APT')
    t = Tenant(name='Doc Tester', apartment=a)
    c = Contract(
        tenant=t, 
        apartment=a, 
...
def test_save_contract_pdf_success(db_with_contract, tmp_path, app_context):
    ...
    assert saved_filename.startswith(f"contract_{contract_id}_")
