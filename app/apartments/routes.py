...
def list_apartments():
    ...
    return render_template('list.html', apartments=apartments, title='Wohnungsübersicht')
...
def create_apartment():
    ...
    if form.validate_on_submit():
        # Prüfen, ob Wohnungsnummer bereits existiert
        ...
        if existing_apartment:
            ...
            return render_template('form.html', form=form, title='Neue Wohnung anlegen')
    ...
    # Für GET-Requests oder wenn Validierung fehlschlägt
    return render_template('form.html', form=form, title='Neue Wohnung anlegen')
...
def edit_apartment(apartment_id):
    ...
    form = ApartmentForm(obj=apartment) # Formular mit bestehenden Daten füllen

    if form.validate_on_submit():
        # Prüfen, ob die neue Nummer bereits von einer *anderen* Wohnung verwendet wird
        new_number = form.number.data
        if new_number != apartment.number:
            existing_apartment = Apartment.query.filter(Apartment.id != apartment_id, Apartment.number == new_number).first()
            if existing_apartment:
                flash(f'Eine andere Wohnung mit der Nummer "{new_number}" existiert bereits.', 'warning')
                # Rendere Template erneut mit den gerade eingegebenen (aber fehlerhaften) Daten
                return render_template('form.html', form=form, title='Wohnung bearbeiten', instance_id=apartment_id)
        
        # Daten aus dem Formular in das apartment Objekt übertragen
    ...
    # Für GET-Requests oder wenn Validierung fehlschlägt
    # Formular wird mit den Objekt-Daten (GET) oder den fehlerhaften Daten (POST) gerendert
    return render_template('form.html', form=form, title='Wohnung bearbeiten', instance_id=apartment_id) 